...
$$
\begin{aligned}
  \dot \boldsymbol{x} &= -\frac{1}{\tau}(\boldsymbol x - \boldsymbol \lambda) + \sqrt{g}\xi(t), \\
...
```{julia lcghist, fig.cap="\\label{fig:figs}plotting example"}
rng = LCG(1000)
...
Heading on to the study of interactions through spatial movement, we look into the following process: We consider two identical independent OU processes and initialize them randomly according to their stationary distributions (which as one can see above, is $\mathcal N(\boldsymbol\lambda, g\tau/2)$). We allow these processes to evolve in time until they enter each other's "interaction range", defined as a distance $\epsilon$ from one another. At this moment, one terminates the process and records the position and time of the event.

```{julia encountertimehist, fig.cap="\\label{fig:encountertimehist}plotting example"}
rng = LCG(1000)
histogram(PRNG.rand(rng, 100000))
```